 ...
 * פונקציה זו קוראת את הקובץ, מזהה את הסוג שלו ומבצעת את הטעינה
 */
...
/**
 * פענוח טקסט CSV למערך של אובייקטים
 * מטפל נכון בפסיקים בתוך מירכאות (למשל: "כהן, משה")
 */
function parseCSV(csvText) {
    ...
    // זיהוי ודילוג על שורת כותרת אם קיימת
    ...
    dataLines.forEach((line) => {
        // Regex מורכב לפיצול CSV תוך התעלמות מפסיקים בתוך מירכאות
        ...
        if (values.length >= 10) {
            ...
            students.push({
                ...
                age: values[4] || '',
                className: values[5] || '', 
                groupName: '', // לא קיים בפורמט הנוכחי
                street: values[6] || '', 
                houseNum: values[7] || '', 
                city: values[8] || '', 
                phone: values[9] || '', 
...
/**
 * עיבוד נתוני CSV וקריאה לפונקציית היצירה
 */
function importStudentsFromCSV(studentData) {
    ...
    studentData.forEach(data => {
        const [ok, msg] = addStudent(data, true); // true = skipSave (נשמור בסוף)
        if (ok) {
            importedCount++;
        } else {
            skippedCount++;
...
/**
 * ייצוא רשימת הבחורים ל-CSV כולל מיקום שיבוץ
 */
function exportStudentsToCSV() {
  const BOM = "\uFEFF"; // סימון BOM כדי שאקסל יזהה עברית
